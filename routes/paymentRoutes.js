...
// ── GET /api/payments  –  All payments (Admin/Manager) ────────────
...
// ── GET /api/payments/summary  –  Revenue summary (Admin/Manager) ─
router.get('/summary', protect, checkRole('Admin', 'Manager'), async (req, res) => {
    try {
        ...
        const [all, thisM, lastM] = await Promise.all([
            ...
            Payment.aggregate([
                { $match: { transactionDate: { $gte: thisMonth, $lt: nextMonth } } },
                { $group: { _id: null, total: { $sum: '$amount' }, count: { $sum: 1 } } }
            ]),
            Payment.aggregate([
                { $match: { transactionDate: { $gte: lastMonth, $lt: thisMonth } } },
                { $group: { _id: null, total: { $sum: '$amount' }, count: { $sum: 1 } } }
            ])
        ]);
        ...
        res.status(200).json({
            ...
            totalPayments: all[0]?.count || 0,
            thisMonthRevenue: thisM[0]?.total || 0,
            thisMonthCount: thisM[0]?.count || 0,
            lastMonthRevenue: lastM[0]?.total || 0,
            lastMonthCount: lastM[0]?.count || 0
        });
    } catch (err) {
...
// ── PUT /api/payments/:id/complete  –  Mark as Completed (Admin) ──
